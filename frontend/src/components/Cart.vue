...
export default {
  ...
  computed: {
    ...
    // Filter out PWD and Senior Citizen promotions
    filteredPromotions() {
      if (!this.activePromotions || !Array.isArray(this.activePromotions)) {
        return [];
      }
      
      return this.activePromotions.filter(promotion => {
        const name = (promotion.name || '').toLowerCase();
        const description = (promotion.description || '').toLowerCase();
        
        // Exclude promotions that contain PWD or Senior Citizen keywords
        const isPWD = name.includes('pwd') || description.includes('pwd') || 
                      name.includes('person with disability') || description.includes('person with disability');
        const isSenior = name.includes('senior') || description.includes('senior') || 
                        name.includes('citizen') || description.includes('citizen');
        
        return !isPWD && !isSenior;
      });
    }
  },
  methods: {
    ...
    async applyPromoCode() {
      ...
      try {
        ...
        if (result.success) {
          ...
          // Check if this is a PWD or Senior Citizen promotion
          if (this.isPWDSeniorPromotion(promotion)) {
            this.promoError = 'This promotion is not available for online orders. Please present your valid ID at the store for verification.';
            // Remove the promotion immediately
            await this.removePromotionFromCart(promotion.id);
            return;
          }
    ...
    // Check if promotion is PWD or Senior Citizen
    isPWDSeniorPromotion(promotion) {
      ...
      const isPWD = name.includes('pwd') || 
                   ...
                   code.includes('pwd') ||
                   name.includes('person with disability') || 
                   description.includes('person with disability');
    ...
    async autoApplyBestPromotion() {
      try {
        ...
        if (!this.filteredPromotions || this.filteredPromotions.length === 0) return;
    ...
    getItemDiscount(item) {
      ...
      if (CART_DEBUG) console.log('🎁 Available promotions:', this.filteredPromotions.length);
    ...
    getApplicablePromotionForItem(item) {
      ...
      if (CART_DEBUG) console.log('📋 Filtered promotions count:', this.filteredPromotions.length);
      
      // Check if any active promotion applies to this item
      for (const promotion of this.filteredPromotions) {
  ...
  async mounted() {
    ...
    try {
      ...
      console.log('✅ Filtered promotions (excluding PWD/Senior):', this.filteredPromotions.length);
